...
`dev-snapshot-2026-09-13.zip` 是 2026-09-13 的一致性事务快照，包含：

- `database.sql`：21 张业务表的结构与数据，MySQL 8 格式。
- `storage/documents/`：数据库引用的原始 PDF，保留原存储文件名。
- `manifest.json`：每张表的行数、原文件名、资料 SHA-256。

数据库含 9 个知识库、610 个节点、844 条关系、401 个文本片段，及任务 #63 的抽取、分类、复审和证据记录。#63 已完成；候选表还保留旧开发测试残留，统计接口会排除不存在的任务，不应把候选总行数视为当前待复审数。
...
```powershell
Expand-Archive -LiteralPath database/dev-snapshot-2026-09-13.zip -DestinationPath database/extracted
Get-FileHash database/dev-snapshot-2026-09-13.zip -Algorithm SHA256
```

Linux/macOS 可用 `unzip database/dev-snapshot-2026-09-13.zip -d database/extracted`。
...
SELECT COUNT(*) AS nodes FROM knowledge_nodes; -- 610
SELECT COUNT(*) AS edges FROM knowledge_edges; -- 844
SELECT COUNT(*) AS libraries FROM knowledge_libraries; -- 9
```
...
模板跳过只针对旧 PHP 表的 V2 迁移，使用 V1/V3/V4/V5/V6；适用于本快照。不要在没有旧 PHP 表的数据库上使用 V2。快照包含初始化标记，启动不会重新插入 TCP/UDP 演示知识。
...
访问 http://127.0.0.1:5173 。检查知识库 9 个、数据洞察 610 节点/844 关系/待复审 0，处理中心 #63 已完成，AI 复审可展开原文证据。
...
- 验证命令：前端 `npm run build`；后端 `mvnw.cmd test`。本次代码前端构建通过，后端最近一轮 132 项测试通过。
- 后端改动后需重新构建/启动，前端开发服务支持热更新。
- 原 PDF 已在快照中，仅保存一份；根目录临时副本没有重复提交。
