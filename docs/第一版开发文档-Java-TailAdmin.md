...
> 2026-09-13 用户变更：取消人工审核，改为 AI 复审后自动入库。本变更覆盖本文原有人工审核要求与手动确认交互；最新流程、接口、保障和验收见《AI自动复审入库.md》。
...
- AI 问答以知识库为事实来源，但分两层：**主题与当前节点或本地原文无关时才拒答**（返回固定句子「当前知识库中没有足够资料支持该问题。」并置证据不足标记）；**主题相关时不得拒答**——资料写到的事实必须标 `[编号]` 引用，资料没写到的部分由模型用自己的知识讲解、延伸、对比和举例，用于补足教材知识点有限的情况，这类补充内容不得标注引用、不得声称来自资料（2026-09-14 用户决策）。
...
4. 用户确认某项后：
   - 更新 URL：`/?node={id}&depth=1&mode=focus`；
   - 请求节点邻域子图；
   - 画布摄像机平滑聚焦命中节点；
   - 命中节点高亮，非邻域节点不加载；
   - 右侧 AI 上下文切换到该节点；
...
- 图谱模式与深度固定绑定：局部固定 2 层、聚焦固定 1 层，工具栏不提供深度切换；
...
1. 标题：“AI 知识助手”；桌面端默认折叠（打开即为空对话），支持新对话和折叠。
2. 当前知识节点卡片：名称、类型、所属知识库、定义摘要、“查看节点详情”。
3. 快捷提问：
   - 通俗解释；
   - 与相邻节点对比；
   - 生成练习题。
4. 对话区：用户消息、AI 消息、错误消息；仅存在于内存，关闭面板即丢弃（2026-09-14 无历史临时模式）。
...
- 选择节点：打开面板即为空对话；不加载、也不创建任何历史会话。
- 切换节点：立即清空旧节点消息并中断在途请求；旧节点消息不保留。
- 关闭面板、离开图谱页或刷新页面：消息全部丢弃，再次打开为空对话。
- “新对话”只清空当前临时对话；本流程不保存会话，因此不存在需要删除的历史记录。
- 临时历史只在同一次打开期间生效（最多 10 条 user/assistant），随每次请求传给模型，不写入数据库。
...
- 点击引用在问答面板内打开原文定位卡片，展示资料、页/幻灯片/图片单元和摘录；再次点击同一引用或点击关闭按钮可收起，点击另一引用切换内容；新对话、切换节点、关闭面板或离开页面时自动清除定位卡片。
...
任务必须支持增量落库与断点续跑：AI 抽取按批落库（`extraction_batch_results`，指纹只由批次内片段内容与顺序决定、不含 chunkId），失败/取消/中断后重试只补齐缺失批次，已落库批次不得重复调用模型；重新解析导致 chunkId 变化时按位置复用并重映射证据编号，片段内容变化则该批作废重抽；AI 复审结论落候选行，重跑时只复用同一模型的既有结论。任务成功进入 `AWAITING_REVIEW` 时清除断点数据；显式重抽（`force=true`）必须先作废断点数据。解析产物完整（有片段且无 `NEEDS_OCR`/`OCR_SKIPPED`/`OCR_FAILED` 单元）时重试不再删除文档单元、直接从抽取阶段续跑；图片未识别时仍须重新解析与 OCR，否则配置视觉模型后重试无法补全知识缺口。详见 `docs/增量落库与断点续跑.md`。
...
- OCR：通过 `OcrProvider` 接口隔离实现。当前实现为视觉模型通道（`vision=true` 的模型档案承担图片识别，用户多模型决策）；本地 Tesseract 等实现位置保留，安装运行时和中文模型前必须征得用户同意。`transcribe(image, imageFormat)` 的 `imageFormat` 是**裸格式名**（`png` / `jpeg` / `webp`），调用方不得传 `image/png` 这类带 `image/` 前缀的 MIME：拼进 data URL 会变成 `data:image/image/png`，视觉模型会一律判为不支持的图片（REV-017）。
- 知识载体与格式无关：PDF 页、幻灯片、ZIP 图片一律先成为 `document_units`，再分段为 `document_chunks`，问答检索只读这套原文分段；知识图谱是导航骨架，不是全文的替代。任何入口（PDF / PPT / ZIP 图片）都必须走同一条链路，不得为某种格式另建旁路。
- 图片未被识别时不得静默丢失：单元状态区分 `OCR_SKIPPED`（没有视觉识别通道）与 `OCR_FAILED`（通道存在但调用失败或返回空文本）。整份资料没有任何分段时，任务以 `DOCUMENT_OCR_REQUIRED` / `DOCUMENT_OCR_FAILED` 失败并给出可操作提示，保留原始文件与图片单元以便配置模型后重试；不得笼统报成 `DOCUMENT_NO_EXTRACTABLE_TEXT`。部分图片未识别时仍照常抽取入库，但必须在任务上留下「本次入库的知识不完整」的告警。
...
> 2026-09-14（CP01）新增，支撑增量落库与断点续跑（§8.4）。

```text
id, job_id, batch_index, batch_signature, chunk_ids_json, payload_json,
entity_count, relation_count, model, created_at, updated_at
唯一键 (job_id, batch_index)；随 ingestion_jobs 级联删除
```

- `batch_signature`：批次内片段条数、每条长度与内容的 SHA-256，不含 chunkId，故重新解析后仍可复用；内容变化即失效。
- `chunk_ids_json`：落库当时该批真实 chunkId 顺序，用于复用时的证据编号位置重映射。
- `payload_json`：校验后的规范化抽取结果，复用时重新走一遍校验。

### 11.3 AI 对话表

> 2026-09-14 用户决策：节点问答改为**无历史临时模式**。`chat_sessions` / `chat_messages` **为兼容保留，当前问答流程不读写**；已有历史数据保留、不清理，建表定义保留如下。
...
2026-09-14 起为无历史临时模式，仅一条无状态路由（旧 `chat/sessions` 系列路由已移除）：

```text
POST /api/nodes/{nodeId}/chat
```

提问请求（`history` 为同一次打开期间的临时历史，最多 10 条，role 仅允许 user/assistant）：
...
```json
{
  ...
  "history": [
    ...
    { "role": "assistant", "content": "面向连接的可靠传输层协议。" }
  ]
}
```
...
- 文件已删除时不能继续解析；
- 图谱接口不返回指向不存在节点的孤儿边；
- 长任务必须可断点续跑：已完成的抽取批次与复审结论不得因后续失败而作废或重做（§8.4）。
...
- 实现节点绑定的问答面板（桌面默认折叠，打开即为空对话）；
- 实现基于节点、邻域的图谱上下文检索；
- 实现问答、证据不足处理，以及原文分段检索、真实引用与原文定位；PDF 原文按页/单元完整保存为 `document_units` 和 `document_chunks`，图谱只作导航而非全文替代。回答策略分两层（2026-09-14 用户决策）：主题与当前节点或原文分段都无关时返回固定拒答句；主题相关时，资料事实标 `[编号]`，资料没写到的部分由模型补充讲解、延伸与举例且不挂引用。显式问题先按完整词组检索已完成且在用的资料分段；完整中文词组因 PDF 排版未命中时，再回退二字片段检索；代词/承接式问题（含「通俗解释」「与相邻节点对比」「生成练习题」）回退当前节点证据，避免图谱节点错配或快捷问题无关键词导致的误判；
- 完成快捷提问；**2026-09-14 决策取消服务端会话持久化，改为无历史临时模式**（同一次打开期间可连续追问，临时历史随请求传给模型）。

验收：选择 TCP 后提问“它是什么”，回答基于图谱资料；追问“它和 HTTP 是什么关系”能复用本轮临时上下文；关闭面板或刷新后消息为空；`chat_sessions` / `chat_messages` 行数不变。
...
- [ ] 桌面端默认折叠；打开面板即为空对话。
- [ ] 切换节点后立即清空旧消息并中断旧请求。
- [ ] 关闭面板、离开页面或刷新后消息为空，且服务端会话表行数不变。
- [ ] 同一轮打开期间连续追问能复用临时上下文。
- [ ] 主题与当前节点及本地原文都无关时，不编造答案（返回固定句子 + 证据不足标记）。
- [ ] 主题相关但资料未覆盖时，仍会讲解、延伸与举例，且补充内容不挂引用、不声称来自资料。
- [ ] 文档中的提示注入文本不会改变系统行为。
